...
void Heal::operator()(Obj & obj, std::vector<sharedObj>& objList)
{
	// リンク切れ
	...
	{
		// 再生します
		obj._effectID = PlayEffekseer2DEffect(EFFECT_ID(EFFECT::HEAL));
	...
	SetPosPlayingEffekseer2DEffect(obj._effectID,
		...
		static_cast<float>((*target)._pos.y + (*target)._size.y / 2),
		0);
