...
UniqueBase ResultScene::Update(UniqueBase own)
{
	...
	lpSceneMng.AddDrawQue({ _resultLogo,lpSceneMng.ScreenCenter.x,lpSceneMng.ScreenCenter.y-50.0,0.0,1.0,0.0,0,LAYER::BG,DX_BLENDMODE_NOBLEND,255 });
	if (_cnt >= 300)
	{
		lpSceneMng.AddDrawQue({ _resultTex,lpSceneMng.ScreenCenter.x,lpSceneMng.ScreenSize.y - 100,0.0,1.0,0.0,0,LAYER::UI,DX_BLENDMODE_NOBLEND,255 });
	}
